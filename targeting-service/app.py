...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            return jsonify({"error": "Serviço de autenticação indisponivel"}), 503 # Service Unavailable
...
@require_auth
def create_rule():
    """ Cria uma nova regra de segmentacao para uma flag """
...
def get_rule(flag_name):
    """ Busca uma regra de segmentacao pelo nome da flag """
    conn = None
    cur = None
    try:
        conn = pool.getconn()
...
def update_rule(flag_name):
    """ Atualiza a regra de segmentacao de uma flag """
    data = request.get_json()
    if not data:
    ...
    if not fields:
        return jsonify({"error": "Pelo menos um campo ('rules', 'is_enabled') é obrigatorio"}), 400
...
def delete_rule(flag_name):
    """ Deleta a regra de segmentacao de uma flag """
    conn = None
    cur = None
    try:
        conn = pool.getconn()
