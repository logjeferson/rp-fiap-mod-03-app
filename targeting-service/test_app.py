...
def test_create_rule_success(client, mock_auth, mock_db):
    ...
    response = client.post(
        "/rules", json=payload, headers={"Authorization": "Bearer token"}
    )
...
def test_get_rule_success(client, mock_auth, mock_db):
    ...
    response = client.get(
        "/rules/botao_azul", headers={"Authorization": "Bearer token"}
    )
...
def test_update_rule_success(client, mock_auth, mock_db):
    ...
    response = client.put(
        "/rules/botao_azul", json=payload, headers={"Authorization": "Bearer token"}
    )
...
def test_delete_rule_not_found(client, mock_auth, mock_db):
    ...
    response = client.delete(
        "/rules/flag_fantasma", headers={"Authorization": "Bearer token"}
    )
